...
def lambda_handler(event, context):
    ...
    clean_file_name = image_file_name.split('/')[1]
    new_file_name = 'graded/' + clean_file_name.split('.')[0] + '_graded.' + clean_file_name.split('.')[1]
    
    cv2.imwrite("/tmp/" + clean_file_name, paper)
    s3.put_object(Bucket=bucket, Key=new_file_name, Body=open("/tmp/" + clean_file_name, "rb").read())